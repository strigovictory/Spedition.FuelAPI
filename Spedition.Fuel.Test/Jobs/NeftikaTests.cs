...
public class NeftikaTests : AssertHelper
{
    public NeftikaTests(ITestOutputHelper output) : base(output)
    {
    }
    ...
    public async Task GetTransactionsTest()
    {
        ...
        var mockOptions = new Mock<IOptions<NeftikaConfig>>();
        var baseUrl = Configuration.GetSection("Neftika:BaseUrl").Value;
        mockOptions.SetupGet(options => options.Value).Returns(new NeftikaConfig { BaseUrl = baseUrl });
        var jobItem = new NeftikaService(Environment, fuelRepositories, Configuration, Mapper, null, null, null, null, null, mockOptions.Object);
        ...
        var transactions = jobItem?.GetType()?.GetProperties(BindingFlags.NonPublic | BindingFlags.Instance)?
            .FirstOrDefault(prop => prop.Name == "ItemsToMapping").GetValue(jobItem) as List<NeftikaTransaction>;
