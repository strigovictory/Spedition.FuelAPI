...
public class ParserLider : FuelParserBase<FuelTransactionLider, FuelTransactionShortResponse, NotParsedTransaction, FuelTransaction>
{
    public ParserLider(
    ...
    : base(env, fuelRepositories, configuration, mapper, countryService, currencyService, eventsTypeService, divisionService, carService, OuterLibrary.EPPlusShort)
    {
        ProvidersId = new List<int> { 17 };
    }
    ...
    protected override async Task<FuelTransaction> MappingParsedToDB(IParsedItem parsedReportsItem)
    {
        ...
        if (parsedReportsItem == null || parsedReportsItem is not FuelTransactionLider parsedTransaction)
        {
            $"Ошибка - несоответствие транзакции типу {nameof(FuelTransactionLider)} !".LogError(GetType().Name, nameof(MappingParsedToDB));
            return null;
        }
