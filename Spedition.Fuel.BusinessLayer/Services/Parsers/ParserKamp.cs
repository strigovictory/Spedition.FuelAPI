...
public class ParserKamp : FuelParserBase<FuelTransactionKamp, FuelTransactionShortResponse, NotParsedTransaction, FuelTransaction>
{
    public ParserKamp(
    ...
    protected override async Task<FuelTransaction> MappingParsedToDB(IParsedItem parsedReportsItem)
    {
        ...
        if (parsedReportsItem == null || parsedReportsItem is not FuelTransactionKamp parsedTransaction)
        {
            $"Ошибка - несоответствие транзакции типу {nameof(FuelTransactionKamp)} !".LogError(GetType().Name, nameof(MappingParsedToDB));
            return null;
        }
