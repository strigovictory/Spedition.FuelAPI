...
public class ParserDieselTrans : FuelParserBase<FuelTransactionDieselTrans, FuelTransactionShortResponse, NotParsedTransaction, FuelTransaction>
{
    public ParserDieselTrans(
    ...
    protected override async Task<FuelTransaction> MappingParsedToDB(IParsedItem parsedReportsItem)
    {
        ...
        if (parsedReportsItem is not FuelTransactionDieselTrans parsedTransaction)
        {
            $"Ошибка - несоответствие транзакции типу {nameof(FuelTransactionDieselTrans)} !".LogError(GetType().Name, nameof(MappingParsedToDB));
            return null;
        }
